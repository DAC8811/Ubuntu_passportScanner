...
typedef struct
{
    ...
    unsigned int uInstance;				///< \~chinese 该型号相机在该电脑上的实例索引号，用于区分同型号多相机 \~english The example index number of this model camera on this computer, used to distinguish the same model multiple cameras
} tSdkCameraDevInfo;
...
typedef struct
{
  ...
  unsigned int uSkipModeMask;       ///< \~chinese SKIP模式掩码，为0，表示不支持SKIP 。bit0为1,表示支持SKIP 2x2 ;bit1为1，表示支持SKIP 3x3.... \~english The SKIP mode mask, which is 0, indicates that SKIP is not supported. Bit0 is 1 to indicate that SKIP 2x2 is supported; bit1 is 1 to indicate that SKIP 3x3 is supported....
  unsigned int uBinSumModeMask;     ///< \~chinese BIN(求和)模式掩码，为0，表示不支持BIN 。bit0为1,表示支持BIN 2x2 ;bit1为1，表示支持BIN 3x3.... \~english The BIN (sum) pattern mask, which is 0, indicates that BIN is not supported. Bit0 is 1, indicating that BIN 2x2 is supported; bit1 is 1, indicating that BIN 3x3 is supported....
  unsigned int uBinAverageModeMask; ///< \~chinese BIN(求均值)模式掩码，为0，表示不支持BIN 。bit0为1,表示支持BIN 2x2 ;bit1为1，表示支持BIN 3x3.... \~english The BIN (mean value) mode mask, which is 0, indicates that BIN is not supported. Bit0 is 1, indicating that BIN 2x2 is supported; bit1 is 1, indicating that BIN 3x3 is supported....
  unsigned int uResampleMask;       ///< \~chinese 硬件重采样的掩码 \~english Hardware resampling mask
} tSdkResolutionRange;
...
typedef struct
{
  ...
  unsigned int    uBinSumMode;			///< \~chinese BIN(求和)的模式,范围不能超过tSdkResolutionRange.uBinSumModeMask \~english The BIN (sum) mode cannot exceed the tSdkResolutionRange.uBinSumModeMask
  unsigned int    uBinAverageMode;		///< \~chinese BIN(求均值)的模式,范围不能超过tSdkResolutionRange.uBinAverageModeMask \~english BIN (average) mode, the range cannot exceed tSdkResolutionRange.uBinAverageModeMask 
  unsigned int    uSkipMode;			///< \~chinese 是否SKIP的尺寸，为0表示禁止SKIP模式，范围不能超过tSdkResolutionRange.uSkipModeMask \~english Whether the SKIP size is 0 indicates that the SKIP mode is disabled and the range cannot exceed the tSdkResolutionRange.uSkipModeMask
  unsigned int    uResampleMask;		///< \~chinese 硬件重采样的掩码 \~english Hardware resampling mask
...
typedef struct
{           
    unsigned int  uiTargetMin;		///< \~chinese 自动曝光亮度目标最小值	\~english Auto exposure brightness target minimum
    unsigned int  uiTargetMax;		///< \~chinese 自动曝光亮度目标最大值	\~english Auto exposure brightness target maximum
    unsigned int  uiAnalogGainMin;	///< \~chinese 模拟增益的最小值，单位为fAnalogGainStep中定义 \~english	The minimum value of the analog gain in fAnalog defined in GainStep
    unsigned int  uiAnalogGainMax;	///< \~chinese 模拟增益的最大值，单位为fAnalogGainStep中定义 \~english The maximum value of the analog gain in fAnalog defined in GainStep
    ...
    unsigned int  uiExposeTimeMin;	///< \~chinese 手动模式下，曝光时间的最小值，单位:行。根据CameraGetExposureLineTime可以获得一行对应的时间(微秒),从而得到整帧的曝光时间 \~english The minimum exposure time in manual mode, unit: line. According to CameraGetExposureLineTime can get a row of corresponding time (microseconds) to get the entire frame exposure time
    unsigned int  uiExposeTimeMax;	///< \~chinese 手动模式下，曝光时间的最大值，单位:行 \~english Maximum exposure time in manual mode, unit: line
} tSdkExpose;
...
typedef struct
{
    ...
    unsigned int iPackSize;			  ///< \~chinese 包大小 \~english Packet size
} tSdkPackLength;
...
typedef struct
{
  ...
  char    acDescription[32];  ///< \~chinese 描述信息 \~english description
  unsigned int    iMediaType;         ///< \~chinese 对应的图像格式编码，如CAMERA_MEDIA_TYPE_BAYGR8。 \~english Corresponding image format encoding, such as CAMERA_MEDIA_TYPE_BAYGR8.
} tSdkMediaType;
...
typedef struct
{
    char bMonoSensor;       ///< \~chinese 表示该型号相机是否为黑白相机,如果是黑白相机，则颜色相关的功能都无法调节 \~english Indicates whether this model is a black-and-white camera. If it is a black-and-white camera, color-related functions cannot be adjusted.
    char bWbOnce;           ///< \~chinese 表示该型号相机是否支持手动白平衡功能 \~english Indicates whether this model camera supports manual white balance function
    char bAutoWb;           ///< \~chinese 表示该型号相机是否支持自动白平衡功能 \~english Indicates whether this model camera supports automatic white balance function
    char bAutoExposure;     ///< \~chinese 表示该型号相机是否支持自动曝光功能 \~english Indicates whether this model camera supports auto exposure function
    char bManualExposure;   ///< \~chinese 表示该型号相机是否支持手动曝光功能 \~english Indicates whether this model camera supports manual exposure function
    char bAntiFlick;        ///< \~chinese 表示该型号相机是否支持抗频闪功能 \~english Indicates whether this model camera supports anti-strobe function
    char bDeviceIsp;        ///< \~chinese 表示该型号相机是否支持硬件ISP功能 \~english Indicates whether this model camera supports hardware ISP function
    char bForceUseDeviceIsp;///< \~chinese bDeviceIsp和bForceUseDeviceIsp同时为TRUE时，表示强制只用硬件ISP，不可取消。 \~english When both bDeviceIsp and bForceUseDeviceIsp are TRUE, this means that only the hardware ISP is forced and cannot be cancelled.
    char bZoomHD;           ///< \~chinese 相机硬件是否支持图像缩放输出(只能是缩小)。 \~english Whether the camera hardware supports image scaling output (can only be reduced).
} tSdkIspCapacity;
...
typedef struct
{
  ...
  char          bParamInDevice;         ///< \~chinese 指示该设备是否支持从设备中读写参数组。1为支持，0不支持。 \~english Indicates whether the device supports reading and writing parameter groups from the device. 1 is supported, 0 is not supported.
...
typedef struct 
{
  unsigned int    uiMediaType;    ///< \~chinese 图像格式 \~english Image Format
  unsigned int    uBytes;         ///< \~chinese 图像数据字节数 \~english Total bytes
  ...
  char    bIsTrigger;     ///< \~chinese 指示是否为触发帧 \~english is trigger 
  unsigned int    uiTimeStamp;    ///< \~chinese 该帧的采集时间，单位0.1毫秒 \~english The frame acquisition time, in units of 0.1 milliseconds
  unsigned int    uiExpTime;      ///< \~chinese 当前图像的曝光值，单位为微秒us \~english Exposure of the current image in microseconds us
...
typedef struct sCameraFrame
{
  ...
  unsigned char *          pBuffer;  ///< \~chinese 数据区 \~english Image Data
}tSdkFrame;

/// @ingroup API_GRAB_CB
/// \~chinese 图像捕获的回调函数定义
/// \~english Callback function definition for image capture
//typedef void (WINAPI* CAMERA_SNAP_PROC)(CameraHandle hCamera, unsigned char *pFrameBuffer, tSdkFrameHead* pFrameHead,PVOID pContext);

/// @ingroup API_SETTINGS_PAGE
/// \~chinese 相机配置页面的消息回调函数定义
/// \~english camera configuration page message callback function definition
//typedef void (WINAPI* CAMERA_PAGE_MSG_PROC)(CameraHandle hCamera,unsigned int MSG,unsigned int uParam,PVOID pContext);
...
//typedef void (WINAPI* CAMERA_CONNECTION_STATUS_CALLBACK)(CameraHandle hCamera,unsigned int MSG,unsigned int uParam,PVOID pContext);
