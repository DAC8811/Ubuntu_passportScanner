...
std::string MRZ::OCR(const Mat &img, int type, std::string outputPath) {
    m_mrzImg = findMrzArea(img, type, outputPath);
    ...
    recognize();
    checkMrzStr();
    std::string code = mrzStr();
    try {
        ofstream out(outputPath + "/mrz.txt");
        out << code << endl;
        out.close();
    } catch (...) {
...
int MRZ::getMrzNums(const cv::Mat &img) {
    mrzNum = findMRZNum(img, lineImgs);
    return mrzNum;
}

int MRZ::initNet(std::string modelPath) {
    m_net = readNet(modelPath);
    setNumThreads(22);//多线程并发计算
    //先空算一次，有利于CPU缓存，加速之后的计算
    Mat input = Mat::zeros(32, 32, CV_8UC1);
    Mat blob;
...
int MRZ::segmentChars() {
    ...
    //@wenyang2020 0720
    Mat verticalKernel = getStructuringElement(MORPH_RECT, Size(1, 3));//5

    Mat blackhat;
    morphologyEx(img, blackhat, MORPH_BLACKHAT, horizontalShort);
    //@wenyang2020 0720
    blackhat(Rect(0, 0, 13, img.rows)) = 0;//30
    ...
    //imgBin = imgBin == 0;
    //@wenyang2020 0720
    imgBin(Rect(0, 0, 13, img.rows)) = 0;//30
    ...
    for (int i = 0; i < lineImgs.size(); ++i) {
        ...
        for (Mat contour : contours) {
            Rect rect = boundingRect(contour);
            if ((rect.height > lineImgs[i].rows * 0.35) && (rect.height < lineImgs[i].rows - 2)) {
...
//        imwrite("/sdcard/passport_lib/com.dscvlab.passportreader/images"+to_string(i)+".jpg",tmp);
#ifdef SHOW
        Mat tmp = lineImgs[i].clone();
        for (Rect rect : rects)
...
//            Mat input;
//            resize(vec[k], input, Size(32, 32));
//            imgs.push_back(input);
//            if ((k+1) % 44 == 0 || k == vec.size()-1) {
//                Mat blob;
//                blobFromImages(imgs, blob, 1.0, Size(32, 32));
//                m_net.setInput(blob);
//                Mat logit = m_net.forward();
//                for (int i = 0; i < imgs.size(); i++) {
//                    int index = 0;
//                    float max = logit.at<float>(i, 0);
//                    for (int j = 1; j < 37; ++j)
//                    {
//                        if (logit.at<float>(i, j) > max)
//                        {
//                            max = logit.at<float>(i, j);
//                            index = j;
//                        }
//                    }
//                    m_mrzStr.push_back(charTable[index]);
...
//多张图片，加速计算
    ...
    for (vector<Mat> vec : m_charImgMat) {
        ...
        if (charNums.size() == 0) {
            charNums.push_back(vec.size());
        } else {
            charNums.push_back(vec.size() + charNums.back());
        }

        //std::cout << charNums.back() << std::endl;;
        for (Mat input:vec) {
            batchCount++;
            resize(input, input, Size(32, 32));
            imgs.push_back(input);
            if (batchCount % 44 == 0 || batchCount == charNums.back()) {
                Mat blob;
                blobFromImages(imgs, blob, 1.0, Size(32, 32));
                m_net.setInput(blob);
                Mat logit = m_net.forward();
    ...
    //std::cout << "recognize ok..." << std::endl;
    int posShift = 0;
    for (int lineNum : charNums) {

        m_mrzStr.insert(lineNum + posShift, "\n");
        posShift++;
...
void MRZ::checkMrzStr() {
    ...
    if (m_type == 0) {
        ...
        if (line1Str.substr(0, 2) == "CT") {//台湾居民往来大陆通行证
            ...
            for (int i = 20; i < 30; ++i)
                alpha2num(line2Str[i]);
            //*********第三行********//拉丁姓名+4签发机关代码（英文HKGL\MACL或数字）
            for (int i = 0; i < 26; i++)//有待优化
                num2alpha(line3Str[i]);
...
int MRZ::IDNumSegmentChars()
{

    Mat img = inputIDNumImg.clone();
    resize(img,img,Size(675,415));
    img = img(Rect(180, 315, 240, 70));
    img = fadeBackground(img);
#ifdef SHOW
    namedWindow("faded", 0);
	imshow("faded", img);
	waitKey(0);
...
//#endif
    ...
    int horizontalKernalWidth = 16;//50
    Mat horizontalShort = getStructuringElement(MORPH_RECT, Size(horizontalKernalWidth, 1));
    //@wenyang2020 0720
    Mat verticalKernel = getStructuringElement(MORPH_RECT, Size(1, 3));//5
...
//            std::cout << rect.height << std::endl;
            ...
            if (toBePad > 0)
            {
                ...
                rect.width += toBePad;
                rect.x = max(0, rect.x);
                rect.width = min(rect.width, imgBin.cols - 1 - rect.x);
            }
            rects.push_back(rect);
        }
    }
	...
	destroyAllWindows();
#endif
    m_IDNumCharImgMat.clear();
    for (Rect rect : rects)
    {
        m_IDNumCharImgMat.push_back(img(rect));
#ifdef SHOW
        namedWindow("char", 0);
			imshow("char", img(rect));
			waitKey(0);
			destroyAllWindows();
#endif // SHOW
    }
...
void MRZ::IDNumRecognize()
{
    IDNumAns = "";
    for (Mat charImg : m_IDNumCharImgMat)
    {
        int index = 0;
        Mat input;
        resize(charImg, input, Size(32, 32));
        ...
        IDNumAns.push_back(charTable[index]);
    }
}
std::string MRZ::IDNumOCR(const cv::Mat &img) {
    inputIDNumImg = img.clone();
    IDNumSegmentChars();
    IDNumRecognize();
    std::replace(IDNumAns.begin(), IDNumAns.end(), 'O', '0');
    std::replace(IDNumAns.begin(), IDNumAns.end(), 'D', '0');
    std::replace(IDNumAns.begin(), IDNumAns.end(), 'I', '1');
    std::replace(IDNumAns.begin(), IDNumAns.end(), 'Z', '2');
    std::replace(IDNumAns.begin(), IDNumAns.end(), 'U', '0');
    std::replace(IDNumAns.begin(), IDNumAns.end(), 'S', '5');
    std::replace(IDNumAns.begin(), IDNumAns.end(), 'B', '8');
    return IDNumAns;
}
