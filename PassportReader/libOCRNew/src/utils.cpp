...
Mat fadeBackground(Mat src)
{
	...
	Mat dst = Mat::zeros(SrcRow, SrcCol, CV_8UC1);
	//uchar* MatData;
	uchar* r;
	uchar* g;
	uchar* b;
	...
	{
		//MatData = src.ptr<uchar>(j);
		b = rgb[0].ptr<uchar>(j);
		g = rgb[1].ptr<uchar>(j);
		r = rgb[2].ptr<uchar>(j);
		for (int i = 0; i < SrcCol; i++)
		{
			/*if (r[i] < 100 && g[i] < 100 && b[i] < 100)dst.at<uchar>(j, i) = int(0.299 * r[i] + 0.587 * g[i] + 0.114 * b[i]);
			else dst.at<uchar>(j, i) = max(max(r[i], g[i]), b[i]);*/
			dst.at<uchar>(j, i) = max(max(r[i], g[i]), b[i]);
		}
	}
	return dst;
}
