...
int imagetobmp(opj_image_t *image, const char *outfile) {
    ...
    if (image->numcomps >= 3 && image->comps[0].dx == image->comps[1].dx
        ...
        if (image->comps[0].prec > 8) {
            ...
            LOGV("BMP CONVERSION: Truncating component 0 from %d bits to 8 bits\n",
                   image->comps[0].prec);
        } else {
            adjustR = 0;
        }
        if (image->comps[1].prec > 8) {
            ...
            LOGV("BMP CONVERSION: Truncating component 1 from %d bits to 8 bits\n",
                   image->comps[1].prec);
        } else {
            adjustG = 0;
        }
        if (image->comps[2].prec > 8) {
            ...
            LOGV("BMP CONVERSION: Truncating component 2 from %d bits to 8 bits\n",
                   image->comps[2].prec);
        } else {
            adjustB = 0;
        }
...
char jp2_to_bmp(std::string &data, std::string filename) {
    ...
    if (jpeg_version == 2000)
        ...
        //todo: needs test
        Mat temp = imdecode(
                vector<unsigned char>(data.data() + offset, data.data() + data.length() - offset), 1);
